...
final class CalibrationEngine: ObservableObject {
    ...
    /// The screen size used to convert normalized targets to screen coordinates.
    /// Set via `startCalibration(screenSize:)`. Falls back to UIScreen.main.bounds.
    private var calibrationScreenSize: CGSize?

    /// Resets state and begins a new calibration session.
    ///
    /// - Parameter screenSize: The actual content area size used to position
    ///   calibration dots. If nil, falls back to `UIScreen.main.bounds.size`.
    func startCalibration(screenSize: CGSize? = nil) {
        currentPointIndex = 0
        isCalibrating = true
        calibrationResult = nil
        calibrationScreenSize = screenSize
    ...
    func computeCalibration() -> CalibrationResult {
        ...
        // Use the screen size passed during startCalibration (which matches the
        // coordinate space where calibration dots were displayed), or fall back
        // to the full screen bounds.
        let screenSize = calibrationScreenSize ?? UIScreen.main.bounds.size
    ...
    /// the average angle of the error vectors relative to the centroid.
    ///
    /// Points whose vector from the centroid is shorter than `minVectorLength`
    /// are skipped because `atan2` of near-zero vectors produces unreliable
    /// angles (this always excludes the center calibration point).
    private func computeRotation(
        ...
        // Minimum vector length (in points) to trust atan2 direction.
        let minVectorLength: CGFloat = 20.0

        for i in 0..<measured.count {
            let transformedX = measured[i].x * scaleX + offsetX
            let transformedY = measured[i].y * scaleY + offsetY

            // Vectors from centroid to target and to transformed point.
            ...
            // Skip points too close to centroid (atan2 is unreliable).
            let targetLen = sqrt(targetVecX * targetVecX + targetVecY * targetVecY)
            let transLen = sqrt(transVecX * transVecX + transVecY * transVecY)
            guard targetLen > minVectorLength, transLen > minVectorLength else {
                continue
            }
        ...
        // Clamp to ±6° — larger rotations indicate noise, not real misalignment.
        let maxRotation: CGFloat = 0.105
        let avgAngle = totalAngle / count
        return max(-maxRotation, min(maxRotation, avgAngle))
    }
