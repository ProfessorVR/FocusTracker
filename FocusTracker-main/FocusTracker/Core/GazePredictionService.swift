...
final class GazePredictionService {
    ...
    struct PredictionResult {
        /// Final gaze point in screen coordinates (points).
        /// In residual mode this is the fused hybrid point; in absolute mode
        /// it's the CNN's direct screen-space prediction.
        ...
        /// Confidence-weighted blend factor applied to the CNN offset (0-1).
        let confidence: Float

        /// The ARKit geometric baseline that was blended with.
        let arkitBaseline: CGPoint
    }
    ...
    /// When `true` (default), the CNN outputs a *residual* offset (dx_cm, dy_cm)
    /// that is added to the ARKit geometric baseline. When `false`, the CNN
    /// outputs absolute coordinates that are mapped to screen space directly
    /// (the original iTracker behavior, for use before retraining).
    var useResidualMode: Bool = true
    ...
    /// Predicts the on-screen gaze point using CNN with confidence-weighted
    /// fusion against the ARKit geometric baseline.
    ///
    /// - Parameters:
    ///   - frame: The current `ARFrame` containing the camera image.
    ///   - faceAnchor: The detected `ARFaceAnchor` with face geometry.
    ///   - arkitBaseline: The geometric gaze point from ScreenMapper (always computed).
    ///   - eulerAngles: Head orientation (pitch, yaw, roll) in radians.
    ///   - cameraTrackingQuality: ARKit camera tracking quality (0-1).
    /// - Returns: A `PredictionResult` with the fused screen point, or `nil`
    ///   if the model is unavailable or prediction fails.
    func predict(
        frame: ARFrame,
        faceAnchor: ARFaceAnchor,
        arkitBaseline: CGPoint,
        eulerAngles: (pitch: Float, yaw: Float, roll: Float),
        cameraTrackingQuality: Float
    ) -> PredictionResult? {
        ...
        // Generate face grid with Euler angles appended (628 elements).
        ...
        let confidence = computeConfidence(
            ...
            yaw: eulerAngles.yaw,
            cameraTrackingQuality: cameraTrackingQuality
        )

        let finalPoint: CGPoint

        if useResidualMode {
            // CNN outputs a residual (dx_cm, dy_cm). Convert to screen-space
            // offset and blend with the ARKit baseline by confidence.
            let offset = cmResidualToScreenOffset(dxCm: prediction.x, dyCm: prediction.y)
            let fused = CGPoint(
                x: arkitBaseline.x + CGFloat(confidence) * offset.x,
                y: arkitBaseline.y + CGFloat(confidence) * offset.y
            )
            finalPoint = calibration?.apply(to: fused) ?? fused
        } else {
            // Original absolute mode: CNN outputs (x_cm, y_cm) mapped to screen.
            let screenPoint = cmToScreenPoint(xCm: prediction.x, yCm: prediction.y)
            finalPoint = calibration?.apply(to: screenPoint) ?? screenPoint
        }
    ...
    /// - `facegrid`: 628-element (25x25 grid + 3 Euler angles) float array
    ///
    ...
    /// Computes a blend confidence for the CNN residual based on pose
    /// extremeness (70% weight) and ARKit camera tracking quality (30%).
    ///
    /// Confidence approaches 0 at extreme head angles or when camera
    /// tracking degrades, causing the system to fall back to pure ARKit.
    private func computeConfidence(
        ...
        guard isTracked else { return 0.0 }

        // Yaw penalty: linear falloff from 1→0 as |yaw| approaches 40°.
        let maxYaw: Float = 40.0 * (.pi / 180.0)  // 40° in radians
        let yawPenalty = max(0.0, 1.0 - abs(yaw) / maxYaw)

        // Pitch penalty: linear falloff from 1→0 as |pitch| approaches 35°.
        let maxPitch: Float = 35.0 * (.pi / 180.0)  // 35° in radians
        let pitchPenalty = max(0.0, 1.0 - abs(pitch) / maxPitch)

        // Combined pose score: use the worse of the two penalties.
        let poseScore = min(yawPenalty, pitchPenalty)

        // Final: 70% pose + 30% camera quality.
        return 0.7 * poseScore + 0.3 * cameraTrackingQuality
    }

    // MARK: - Coordinate Conversion

    /// Converts a CNN residual (dx_cm, dy_cm) to a screen-space offset in points.
    ///
    /// This is used in residual mode where the CNN predicts a correction
    /// delta rather than absolute coordinates.
    private func cmResidualToScreenOffset(dxCm: Float, dyCm: Float) -> CGPoint {
        let ptsPerCmX = screenSize.width / CGFloat(screenWidthCm)
        let ptsPerCmY = screenSize.height / CGFloat(screenHeightCm)
        return CGPoint(
            x: CGFloat(dxCm) * ptsPerCmX,
            y: CGFloat(dyCm) * ptsPerCmY
        )
    }
