...
final class GazeTracker: NSObject, ObservableObject, ARSessionDelegate {
    ...
    private func buildGazeFrame(from faceAnchor: ARFaceAnchor, at timestamp: TimeInterval) -> GazeFrame {
        ...
        // Always compute the geometric baseline first, then try CNN fusion.
        let arkitMapping = screenMapper.map(lookAtPoint: lookAt, faceTransform: matrix_identity_float4x4)
        let arkitBaseline = arkitMapping.point

        let gazePoint: CGPoint
        let isOnScreen: Bool
        var cnnConfidence: Float = -1
        var gazeSource: GazeSource = .arkit

        if let arFrame = session.currentFrame,
           gazePredictionService.isModelAvailable,
           let prediction = gazePredictionService.predict(
               frame: arFrame,
               faceAnchor: faceAnchor,
               arkitBaseline: arkitBaseline,
               eulerAngles: (pitch: pitch, yaw: yaw, roll: roll),
               cameraTrackingQuality: trackingQuality
           ) {
            gazePoint = prediction.screenPoint
            isOnScreen = prediction.isOnScreen
            cnnConfidence = prediction.confidence
            gazeSource = .hybrid
        } else {
            gazePoint = arkitBaseline
            isOnScreen = arkitMapping.isOnScreen
        }
