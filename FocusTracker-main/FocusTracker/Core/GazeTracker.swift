...
/// Primary ARKit face tracking engine with CNN-based gaze prediction.
///
/// Uses the iTracker CoreML model (when available) to predict gaze points
/// directly from camera imagery, replacing the geometric ScreenMapper.
/// Falls back to geometric projection if the CoreML model is not loaded.
///
...
final class GazeTracker: NSObject, ObservableObject, ARSessionDelegate {
    ...
    /// Whether the CNN model is being used (true) or geometric fallback (false).
    var isUsingCNN: Bool { gazePredictionService.isModelAvailable }
    ...
    /// CNN-based gaze prediction service (primary, when model is available).
    private let gazePredictionService = GazePredictionService()

    /// Geometric screen mapper used as fallback when CNN is unavailable.
    ...
    func startTracking() {
        ...
        if gazePredictionService.isModelAvailable {
            print("[GazeTracker] Using CNN-based gaze prediction (iTracker).")
        } else {
            print("[GazeTracker] CNN model not available. Using geometric ScreenMapper fallback.")
        }
    }
    ...
    /// Applies a calibration result to the active prediction service.
    func setCalibration(_ cal: CalibrationResult) {
        gazePredictionService.setCalibration(cal)
        screenMapper.setCalibration(cal)
    }

    /// Removes calibration from all prediction paths.
    func clearCalibration() {
        gazePredictionService.clearCalibration()
        screenMapper.clearCalibration()
    }
    ...
    ///
    /// Attempts CNN prediction first (iTracker CoreML). If the model is
    /// unavailable or prediction fails, falls back to geometric ScreenMapper.
    private func buildGazeFrame(from faceAnchor: ARFaceAnchor, at timestamp: TimeInterval) -> GazeFrame {
        ...
        let quality: Float = faceAnchor.isTracked ? 1.0 : 0.0

        // --- Gaze Point Prediction ---
        // Try CNN prediction first, fall back to geometric ScreenMapper.
        let gazePoint: CGPoint
        let isOnScreen: Bool

        if let arFrame = session.currentFrame,
           gazePredictionService.isModelAvailable,
           let prediction = gazePredictionService.predict(frame: arFrame, faceAnchor: faceAnchor) {
            // CNN prediction succeeded.
            gazePoint = prediction.screenPoint
            isOnScreen = prediction.isOnScreen
        } else {
            // Fallback: geometric projection using identity transform.
            //
            // We intentionally pass identity rather than faceAnchor.transform.
            // The lookAtPoint from ARKit already encodes gaze direction relative
            // to the face — treating it as a direction vector from the origin
            // (identity) produces a more stable projection than transforming
            // through the absolute face position, which compounds errors from
            // head tracking drift and the hardcoded distance assumptions in
            // ScreenMapper. The CNN path handles this properly when available.
            let mapping = screenMapper.map(lookAtPoint: lookAt, faceTransform: matrix_identity_float4x4)
            gazePoint = mapping.point
