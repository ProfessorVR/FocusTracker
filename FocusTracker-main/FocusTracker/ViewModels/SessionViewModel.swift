...
final class SessionViewModel: ObservableObject {
    ...
    private func processFrame(_ frame: GazeFrame) {
        // If the CNN produced a hybrid result, trust the fused point directly.
        // Otherwise, re-map the geometric lookAtPoint with identity (original behavior).
        let gazePoint: CGPoint
        let onScreen: Bool

        if gazeTracker.isUsingCNN && frame.gazeSource == .hybrid {
            gazePoint = frame.screenPoint
            onScreen = frame.isOnScreen
        } else {
