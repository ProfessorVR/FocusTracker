...
final class SessionViewModel: ObservableObject {
    ...
    private func processFrame(_ frame: GazeFrame) {
        // GazeTracker now produces screen coordinates via CNN (or geometric fallback),
        // so we use the frame's gaze point directly — no re-mapping needed.
        let smoothed = smoother.smooth(
            point: frame.screenPoint,
