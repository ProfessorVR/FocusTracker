...
/// calibration engine, and auto-advances through the 5 target points.
@MainActor
final class CalibrationViewModel: ObservableObject {
    ...
    /// Frames to skip after each dot move, giving the user time to fixate.
    /// At 30 FPS, 15 frames ≈ 0.5 seconds of settling time.
    private let settlingFrames = 15
    ...
    func startCalibration(screenSize: CGSize) {
        ...
        engine.startCalibration(screenSize: screenSize)

        updateTargetPosition(screenSize: screenSize)

        // Give the user time to fixate on the first dot before collecting.
        settlingFramesRemaining = settlingFrames
    ...
    private func handleGazeFrame(_ frame: GazeFrame) {
        ...
        // Skip frames during settling period (user is re-fixating on new dot).
    ...
    private func advanceOrFinish() {
        ...
        if hasMore {
            ...
            // All 5 points collected — stop tracking and compute result.
            gazeTracker.stopTracking()
            cancellables.removeAll()
            finishCalibration()
        }
    ...
    /// Computes the calibration result from all collected samples, persists
    /// it to UserDefaults, and applies it to the local gaze tracker.
    private func finishCalibration() {
        phase = .processing

        let result = engine.computeCalibration()

        // Persist so SessionViewModel can load it on next session start.
        result.save()

        // Apply to this tracker instance too (used if calibration view
        // ever shows a live preview after calibration).
