...
final class CalibrationViewModel: ObservableObject {
    ...
    private func finishCalibration() {
        ...
        let scaleError = abs(1.0 - Double(result.scaleX)) + abs(1.0 - Double(result.scaleY))
        let offsetError = abs(Double(result.offsetX)) + abs(Double(result.offsetY))
        accuracy = max(0, 1.0 - (scaleError + offsetError) / 4.0)
